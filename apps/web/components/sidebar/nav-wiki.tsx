...
import { SidebarGroup as SidebarGroupConfig } from '@/types'

interface NavWikiProps {
...
export function NavWiki({ sidebarConfig = [] }: NavWikiProps) {
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild isActive={pathname === introItem.url}>
              <Link href={introItem.url} onClick={() => setOpenMobile(false)}>
                {introItem.icon}
    ...
    <>
      ...
      {sidebarConfig.map(group => {
              ...
              <SidebarMenu>
                {group.items.map(item => (
                  <SidebarMenuItem key={item.id}>
                    <SidebarMenuButton
                      asChild
                      isActive={pathname === `/wiki/${item.documentId}`}
                      tooltip={group.title || item.documentName}
                    >
                      <Link href={`/wiki/${item.documentId}`} onClick={() => setOpenMobile(false)}>
          ...
          return (
            <SidebarGroup key={group.id}>
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {group.items.map(item => {
                    const url = `/wiki/${item.documentId}`
                    return (
                      <SidebarMenuItem key={item.id}>
        ...
        if (group.type === 'Collapse') {
          const isAnyActive = group.items.some(i => pathname === `/wiki/${i.documentId}`)
